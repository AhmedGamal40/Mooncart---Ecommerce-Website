...
export const RenderRatingStars = (rating) => {
  ...
  const hasMalfStars = rating % 1 !== 0;
  const stars = [];

  for (let i = 1; i <= totalStars; i++) {
    if (i <= fullStars) {
      stars.push(<FaStar key={i} color="#ff8a00" />);
    } else if (hasMalfStars && i === fullStars + 1) {
            ...
            <div className="modal-overlay" onClick={CloseModal}>
              ...
              >
                ...
                <div className="modal-details w-1/2 h-[500px] overflow-y-scroll p-9">
                  ...
                  <button className="close-btn absolute top-0 right-0 w-12 h-12 flex justify-center bg-primary-green text-white" onClick={CloseModal} > 
                    <IoClose  size={30} className="my-auto"/>
                  </button>
                </div>
              </div>
            </div>
